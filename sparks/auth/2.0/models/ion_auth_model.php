...
class Ion_auth_model extends CI_Model
{
	...
	**/
    ...
    public function login_by_provider($provider, $provider_uid) {
        ...
        if (!$user_info) { // user isn't exist in authentications table. then you must register user before
            return FALSE;
        }
    ...
    public function register_by_provider($provider, $provider_uid, $username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if (!$user_id = $this->id_by_email($email)) {// user isn't exist in users table.
            ...
            $data = array(
                'username' => $username,
                'password' => $password,
                "photo" => $additional_data['photoURL'],
                "first_name" => $additional_data['firstName'],
                "last_name" => $additional_data['lastName'],
                "address" => $additional_data['region'],
                'email' => $email,
                'ip_address' => $ip_address,
                'created_on' => time(),
                'last_login' => time(),
                'active' => ($manual_activation === false ? 1 : 0),
                'registered_from' => $additional_data['registered_from']
            );

            if($data["country"] != "") {
                $data["country"] = $this->db->select("country_symbol")->like('country_name', $additional_data['country'])->get("country")->row()->country_symbol;
                $data["city_id"] = $this->db->select("id")->like('city', $additional_data['city'])->get("city")->row()->id;
            }
